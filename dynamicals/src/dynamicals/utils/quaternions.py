...
from .geometry import norm3, norm4
...
def log(Q):
    ...
    return np.concatenate([[np.log(Qnorm)], 
                           np.arccos(w / (Qnorm or 1)) * v / (norm3(v) or 1)])
...
def quat_diff(qi: np.quaternion, qf: np.quaternion, shortest: bool = True) -> np.quaternion:
    """ Returns the quaternion that achieves `qr * qi = qf` using the shorthest path (by default). 
    ...
    """
    if shortest:
        if not quat_is_closest(qi, qf):
            qf = -qf
    return qf * qi.conj()


def quat_is_closest(qi: np.quaternion, qf: np.quaternion) -> np.quaternion:
    """ Returns the quaternion `qf` in its closest representation to `qi`, 
        which is the one achieves `qr * qi = qf` using the shorthest path. 
        See more https://en.wikipedia.org/wiki/Quaternion#Geodesic_norm
