...
def test_admittance_tustin_old():
...
def test_lpfilter():
    """ Test for a 3D signal LP filter with forward Euler discretization of the 
    ...
    # three noisy box signals sampled with step h
    # each will pass through a different admittance
    t, sig = make_noisy_step({1.0: [10, 0, 0], 2.0: [20, 0, 0], 4.0: 0}, T=5.0, h=h)
    
    # calculate the output signal
    sig_fil = fil.compute_signal(sig)
    
    # plot the result
    plt.plot(t, sig, label=r"$u$")
    plt.plot(t, sig_fil, linestyle="-", label=r"$y$")
    plt.legend()
    plt.show()
