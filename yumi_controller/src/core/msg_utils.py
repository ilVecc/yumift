...
def YumiParam_to_YumiCoordinatedRobotState(yumi_param: YumiParam, yumi_state: YumiCoordinatedRobotState = None):
...
def RobotState_to_YumiCoordinatedRobotState(robot_state: RobotState):
    ...
    if "gripper_r" in robot_state.poseName:
        ...
        jac_gripper_r = Jacobian_to_ndarray(robot_state.jacobian[0])
    else:
    ...
    if "gripper_l" in robot_state.poseName:
        ...
        jac_gripper_l = Jacobian_to_ndarray(robot_state.jacobian[1])
    else:
        pose_wrench_l = np.zeros(6)
        jac_gripper_l = np.zeros((6,7))
    yumi_state.pose_wrench = np.concatenate([pose_wrench_r, pose_wrench_l])
    ...
    if "absolute" in robot_state.poseName:
        ...
        jac_abs = Jacobian_to_ndarray(robot_state.jacobian[2])
    else:
    ...
    if "relative" in robot_state.poseName:
        ...
        jac_rel = Jacobian_to_ndarray(robot_state.jacobian[3])
    else:
    ...
    if "elbow_r" in robot_state.poseName:
        ...
        jac_elb_r = Jacobian_to_ndarray(robot_state.jacobian[4])
    else:
    ...
    if "elbow_l" in robot_state.poseName:
        ...
        jac_elb_l = Jacobian_to_ndarray(robot_state.jacobian[5])
    else:
