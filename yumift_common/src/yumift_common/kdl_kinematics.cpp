...
#include <yumift_msgs/YumiKinematics.h>
...
class YumiStatePublisher
{
    ...
    //     -0.0155479, -0.0059908, -0.006814363, -0.02071828, -0.000571185, 0.01696876, 0.0007526933,
...
YumiStatePublisher::YumiStatePublisher(ros::NodeHandle *nh){
    ...
    kinematics_pub = nh->advertise<yumift_msgs::YumiKinematics>("/kinematics", 1);
    joint_states_pub = nh->advertise<sensor_msgs::JointState>("/joint_states", 1);

    // get tree from urdf file for entire yumi
    std::string const PATH = ros::package::getPath("yumift_description"); // find path 
    
    if (!kdl_parser::treeFromFile(PATH + "/urdf/yumift.urdf", yumi_tree)){ // create tree
...
void YumiStatePublisher::update(){
    ...
    // --------------------- Jacobians ----------------------------------------
    yumift_msgs::YumiKinematics kinematics_msg;
    // send joint position 
